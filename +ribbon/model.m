...
Dst = ... Ast3.*(Ab1.*Asa2 - Ab2.*Asa1) ...
      ...
      ... 4*kst.*kb.*ksa.*k2.*vt2.*vl2.*(mub2-musa2) ...
       2*kst.*kb.*ksa.*k2.*(mub2-musa2) ... %%%%%%%%%%%%%
      ; 
Dsa = ... Ssa3.*(Ab1.*Sst2-Ab2.*Sst1) ...
      ...kb.*(nu.*k2 + ksa.^2).*((w2-vl2.*mub2).*(kst.^2 - k2) + 2*k2.*w02) ...
      ...kb.*(vl2.*musa2 - 2*vt2.*k2).*((w2-vl2.*mub2).*(kst.^2 - k2) + 2*k2.*w02) ...
      kb.*(ksa.^2 + nu.*k2).*(k2.*(kt.^2-kst.^2) + (1./(1-nu)).*(kl.^2-kb.^2).*(kst.^2-k2) ) ... %%%%%%%%%%%%%
      ...kb.*(vl2.*musa2 - 2*vt2.*k2).*( must2.*(w2 - vl2.*mub2) + 2*k2.*(vl2.*mub2 - vt2.*must2) ) ...
      ...kb.*(vl2.*musa2 - 2*vt2.*k2).*( vl2.*mub2.*(2*k2-must2) + must2.*(w2-2*vt2.*k2) ) ...
...
Db =  ... Sb3.*(Asa1.*Sst2-Asa2.*Sst1) ...
      ...
      ...ksa.*(vl2.*mub2 - 2*vt2.*k2).*((w2-vl2.*musa2).*(kst.^2 - k2) + 2*k2.*w02) ...
      ksa.*(kb.^2 + nu.*k2).*(k2.*(kt.^2-kst.^2) + (1./(1-nu)).*(kl.^2-ksa.^2).*(kst.^2-k2) ) ... %%%%%%%%%%%%%
      ...ksa.*(vl2.*mub2 - 2*vt2.*k2).*( must2.*(w2 - vl2.*musa2) + 2*k2.*(vl2.*musa2 - vt2.*must2) ) ...
      ...ksa.*(vl2.*mub2 - 2*vt2.*k2).*( vl2.*musa2.*(2*k2-must2) + must2.*(w2-2*vt2.*k2) ) ...
